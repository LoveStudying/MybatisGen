...
public class ConfigHelper {
	...
	public static void saveinitConfig() throws Exception {
		Connection conn = null;
		Statement stat = null;
		ResultSet rs = null;
		try {
			conn = ConnectionManager.getConnection();
			stat = conn.createStatement();
			String sql = String.format("INSERT INTO dbs (name, value) values('%s', '%s')", "名称", "{\"dbType\":\"MySQL\",\"encoding\":\"utf8\",\"host\":\"数据库地址\",\"name\":\"18.18.100.5\",\"password\":\"数据库密码\",\"port\":\"3306\",\"schema\":\"music\",\"username\":\"数据库用户名\"}");
			stat.executeUpdate(sql);
		} finally {
			if (rs != null) rs.close();
			if (stat != null) stat.close();
